...
#define ASSERT_EQUALS_RECORDS(_l,_r, schema, message)			\
  do {									\
    Record *_lR = _l;   printf("LR size :-%zd and %s\n",strlen(_lR->data),_lR->data);			\
    Record *_rR = _r;      printf("RR size :-%zd and %s\n",strlen(_rR->data),_rR->data);                                              \
    ASSERT_TRUE(memcmp(_lR->data,_rR->data,getRecordSize(schema)) == 0, message); \
    int i;	printf("--->1%p<---",&_rR);						\
    for(i = 0; i < schema->numAttr; i++)				\
      {			printf("--->2%p<---",&_rR);							\
        Value *lVal, *rVal;          	printf("--->3%p<---",&_rR);                                   \
		char *lSer, *rSer; 	printf("--->4%p<---",&_rR);\
        getAttr(_lR, schema, i, &lVal); 	if(i == 1) printf("--->1%s<---",lVal->v.stringV);                                 \
        getAttr(_rR, schema, i, &rVal);         if(i == 1) printf("--->2%s<---",rVal->v.stringV);                         \
		lSer = serializeValue(lVal); if(i == 1) printf("--->3%s<---",lSer);\
		rSer = serializeValue(rVal); if(i == 1) printf("--->4%s<---",rSer);\
...
printf("Scan One adn Scan Two 1st Time = %d and %d\n",scanOne,scanTwo);

  while((rc2 = next(sc2, r)) == RC_OK)
    scanTwo++;

printf("Scan Two 2nd time= %d\n",scanTwo);
...
testInsertManyRecords(void)
{
  ...
  for(i = 0; i < numInserts; i++)
    {
      realInserts[i] = inserts[i%10];//realInserts[0] = {1, "aaaa", 3}; a =1 ; b = "aaaa" ; c = 3;
      realInserts[i].a = i;
      //printf("KKKKK1----%s",*(r)->data);
