...
int prevSlot = 0;int prevRecSlot = 0;int count = 0;
// table and manager
RC initRecordManager(void *mgmtData) {
	return RC_OK;
}
RC shutdownRecordManager() {
	prevSlot = 0;prevRecSlot = 0;count = 0;
	return RC_OK;
}
...
RC insertRecord(RM_TableData *rel, Record *record) {
	...
	if(strlen(bm_ph->data) >0 && strlen(bm_ph->data) % PAGE_SIZE == 0){
		printf("Page Number = %d is full . So Appending one more page using ensure Capacity",pageNumber);
		pageNumber = pageNumber + (strlen(bm_ph->data) / PAGE_SIZE) ;
		SM_FileHandle fHandle;
		fHandle.fileName = bm->pageFile;
		int stat = ensureCapacity1(pageNumber,&fHandle);
		printf("Stat = %d \n",stat);
	...
	if(slotForEachRecord == 1){
		...
		rID.slot = 16+prevRecSlot;
		prevRecSlot = rID.slot;
	}
	record->id = rID;
	printf("Slot Number in RID is [%d]\n",record->id.slot);
